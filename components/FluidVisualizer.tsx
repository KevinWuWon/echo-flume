...
const FluidVisualizer: React.FC<Props> = ({ audioMetrics, gain }) => {
  ...
  // Update audio ref on prop change
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    function update() {
      ...
      // Get Audio Data
      const { metrics, gain } = audioRef.current;
      ...
      if (config.BLOOM) {
        ...
        gl.uniform1i(displayMaterial.uniforms.uDithering, 2); // Dithering not implemented yet, skipping
        // Need to load dithering texture if we want it, but for now just ignore or use noise
        // For simplicity, let's skip dithering texture load
      }
      if (config.SUNRAYS)
    ...
    return () => {
      ...
      // Cleanup logic would go here (delete programs, textures, buffers)
    };
  }, []);
